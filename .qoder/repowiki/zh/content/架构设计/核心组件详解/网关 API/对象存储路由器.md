...
## 更新摘要
**变更内容**
- 更新了文件扩展名自动检测机制，支持多种音频格式的智能识别
- 改进了动态文件命名策略，确保文件名的唯一性和可追溯性
- 优化了上传流程，增强了错误处理和日志记录能力
- 新增了详细的音频格式支持说明和文件命名规范
...
- **智能文件扩展名检测**：根据文件内容类型自动识别并设置正确的文件扩展名
- **动态文件命名策略**：生成带时间戳的唯一文件名，确保文件的可追溯性和安全性
- **多格式音频支持**：全面支持 AAC、MP3、WAV、WebM 等音频格式的自动识别和处理
...
该组件采用模块化设计，与系统的其他部分（如认证中间件、沙箱提供程序等）紧密集成，为用户提供了一个安全、可靠、智能化的云端文件存储解决方案。
...
1. **智能文件上传接口** (`POST /api/oss/upload`)
   - 接收音频文件上传
   - **新增**：自动检测文件类型并智能设置扩展名
   - **新增**：动态生成带时间戳的唯一文件名
   - 将文件存储到阿里云 OSS
...
- **AAC 格式**：默认扩展名 `.aac`
- **MP3 格式**：当检测到 `audio/mpeg` MIME 类型时使用 `.mp3`
- **WAV 格式**：当检测到 `audio/wav` MIME 类型时使用 `.wav`
- **WebM 格式**：当检测到 `audio/webm` MIME 类型时使用 `.webm`

### 动态文件命名策略

文件命名采用统一的命名规范：

- **命名模式**：`voice/upload_{timestamp}{extension}`
- **时间戳**：精确到秒的时间戳，确保文件名唯一性
- **扩展名保留**：根据检测到的内容类型自动设置正确的扩展名
- **路径结构**：统一的 `voice/upload_` 目录结构
...
- **文件名生成**：`generate_filename()` - 支持原始文件名扩展名保留
- **错误处理**：完整的异常处理和日志记录
...
**章节来源**
- [backend/app/gateway/routers/oss.py:17-108](file://backend/app/gateway/routers/oss.py#L17-L108)
- [backend/deerflow/utils/oss_upload.py:15-82](file://backend/deerflow/utils/oss_upload.py#L15-L82)
- [backend/packages/harness/deerflow/config/oss_config.py:6-38](file://backend/packages/harness/deerflow/config/oss_config.py#L6-L38)
...
**图表来源**
- [backend/app/gateway/routers/oss.py:35-108](file://backend/app/gateway/routers/oss.py#L35-L108)
...
Router->>Router : 检测文件类型并智能设置扩展名
Note over Router : 智能扩展名检测<br/>audio/mpeg -> .mp3<br/>audio/wav -> .wav<br/>audio/webm -> .webm<br/>默认 -> .aac
Router->>Router : 生成带时间戳的唯一文件名
Router->>Utils : 上传文件到 OSS
Utils->>Config : 获取认证信息
Config-->>Utils : 返回访问凭据
Utils->>OSS : 上传文件
OSS-->>Utils : 返回上传结果
...
**症状**：上传成功但文件扩展名不符合预期
**排查步骤**：
1. 检查客户端发送的 `Content-Type` 头部
2. 验证音频文件的实际编码格式
3. 确认 MIME 类型与实际文件格式匹配
4. 查看服务器日志中的扩展名检测结果
...
对象存储路由器为 DeerFlow 项目提供了一个完整、安全、高效且智能化的云端文件存储解决方案。该系统具有以下优势：

1. **智能扩展名检测**：自动识别音频格式并设置正确的文件扩展名
2. **动态文件命名策略**：生成带时间戳的唯一文件名，确保文件的可追溯性
3. **模块化设计**：清晰的分层架构便于维护和扩展
4. **安全性强**：完善的认证授权机制和数据保护措施
5. **性能优异**：优化的上传流程和资源管理
6. **易于使用**：简洁的 API 设计和详细的文档支持

通过集成阿里云 OSS 和智能文件处理机制，系统能够满足各种规模的应用需求，从个人开发者到企业级应用都能找到合适的使用场景。未来可以考虑增加更多云存储提供商的支持，以及更丰富的文件处理功能。
...
# 上传 MP3 音频文件到 OSS（自动检测为 .mp3）
curl -X POST "http://localhost:8001/api/oss/upload" \
  -H "Content-Type: audio/mpeg" \
  -F "file=@/path/to/audio.mp3"

# 上传 WAV 音频文件到 OSS（自动检测为 .wav）
curl -X POST "http://localhost:8001/api/oss/upload" \
...
- **认证测试**：验证 JWT 认证机制
- **权限测试**：测试权限控制功能
- **扩展名检测测试**：验证智能文件扩展名检测功能

**章节来源**
- [backend/test_apis.py:157-196](file://backend/test_apis.py#L157-L196)
- [backend/tests/test_uploads_router.py:1-674](file://backend/tests/test_uploads_router.py#L1-L674)
- [backend/docs/CONFIGURATION.md:309-377](file://backend/docs/CONFIGURATION.md#L309-L377)