"""CRUD 服务层 — 从 deerflow/roleplay/services.py 迁移"""
...
from app.models import SceneRow, EvaluationRow, CourseRow, CourseRecordRow
...
class SceneService:
    ...
    async def create_scene(data: dict):
        async with get_db() as session:
            ...
            # prompt_template 存入 metadata_json
...
class PracticeRecordService:
    ...
    async def get_practice_records(
        ...
        async with get_db() as session:
            query = (
                select(CourseRecordRow, CourseRow.course_name)
...
class DialogDetailService:
    ...
    async def create_dialog(data: dict):
        from app.models import DialogDetailRow
        async with get_db() as session:
...
class StatisticsService:
    ...
    async def get_scene_stats(course_id: int = None):
        ...
        async with sf() as session:
            query = select(
                CourseRecordRow.course_id,
                func.count(CourseRecordRow.id).label("practice_count"),
                func.avg(CourseRecordRow.total_score).label("avg_score")
            ).group_by(CourseRecordRow.course_id)
            if course_id:
                query = query.where(CourseRecordRow.course_id == course_id)
            result = await session.execute(query)
            return result.all()
    ...
    async def get_leaderboard(limit: int = 10):
        ...
        async with sf() as session:
            query = select(
                CourseRecordRow.user_name,
                func.count(CourseRecordRow.id).label("practice_count"),
                func.avg(CourseRecordRow.total_score).label("avg_score")
            ).group_by(CourseRecordRow.user_name)\
             .order_by(desc(func.avg(CourseRecordRow.total_score)))\
             .limit(limit)
            result = await session.execute(query)
            return result.all()
