"""Roleplay API 路由 — 从 app/gateway/routers/roleplay.py 迁移"""
...
from app.services import SceneService, EvaluationService, PracticeRecordService, StatisticsService, CourseService
...
class CourseCreate(BaseModel):
    course_name: str
    course_type: Optional[int] = Field(default=1, alias="courseType")
    ...
    practice_mode: Optional[str] = Field(default="text", alias="practiceMode")
    automatically: Optional[int] = Field(default=0, alias="automatically")
    ...
    status: Optional[int] = Field(default=0, alias="status")
...
class CourseUpdate(BaseModel):
    ...
    simulated_role_id: Optional[int] = Field(default=None, alias="simulatedRoleId")
    practice_mode: Optional[str] = Field(default=None, alias="practiceMode")
    automatically: Optional[int] = Field(default=None, alias="automatically")
    difficulty: Optional[int] = Field(default=None, alias="difficulty")
...
async def create_scene(scene: SceneCreate):
    data = scene.model_dump(by_alias=False)
    result = await SceneService.create_scene(data)
...
async def update_scene(scene_id: int, scene: SceneUpdate):
    data = scene.model_dump(exclude_none=True, by_alias=False)
    result = await SceneService.update_scene(scene_id, data)
    if not result:
        raise HTTPException(status_code=404, detail="Scene not found")
...
async def create_course(course: CourseCreate):
    data = course.model_dump(by_alias=False)
    result = await CourseService.create_course(data)
...
async def update_course(course_id: int, course: CourseUpdate):
    data = course.model_dump(exclude_none=True, by_alias=False)
    result = await CourseService.update_course(course_id, data)
    if not result:
        raise HTTPException(status_code=404, detail="Course not found")
    ...
    try:
        ...
        try:
            ...
            if "not found in config" in str(ve):
                import logging
                logging.getLogger("roleplay").warning(f"场景指定模型 '{model_name}' 不在 config 中，已自动切换为默认模型")
        ...
        if response and response.generations and response.generations[0]:
            ...
            return {"success": True, "summaryText": summary_text, "categories": categories}
        else:
...
async def get_evaluations(thread_id: Optional[str] = None, user_id: Optional[str] = None):
    ...
    return {"evaluations": [
        {
            "id": e.id, "thread_id": e.thread_id, "run_id": e.run_id,
            "scene_id": e.scene_id, "user_id": e.user_id,
            "round_number": e.round_number, "total_score": e.total_score,
            "dimension_scores": e.dimension_scores, "strengths": e.strengths,
            "improvements": e.improvements, "summary": e.summary,
...
async def get_evaluation(eval_id: str):
    ...
    return {
        "id": evaluation.id, "thread_id": evaluation.thread_id, "run_id": evaluation.run_id,
        "scene_id": evaluation.scene_id, "user_id": evaluation.user_id,
        "round_number": evaluation.round_number, "total_score": evaluation.total_score,
        "dimension_scores": evaluation.dimension_scores, "strengths": evaluation.strengths,
        "improvements": evaluation.improvements, "summary": evaluation.summary,
        "status": evaluation.status, "created_at": evaluation.created_at.isoformat() if evaluation.created_at else None
    }


@router.post("/evaluations", summary="创建评估")
async def create_evaluation(evaluation: EvaluationCreate):
    result = await EvaluationService.create_evaluation(evaluation.model_dump())
...
async def update_evaluation(eval_id: str, evaluation: EvaluationUpdate):
    result = await EvaluationService.update_evaluation(eval_id, evaluation.model_dump(exclude_none=True))
    if not result:
        raise HTTPException(status_code=404, detail="Evaluation not found")
...
async def get_practice_records(
    ...
    return {"records": [
        {
            "recordId": r["record_id"], "courseId": r["course_id"],
            "userName": r["user_name"], "totalScore": r["total_score"],
            "courseName": r["course_name"], "sceneName": r["scene_name"],
            "startTime": r["start_time"], "endTime": r["end_time"],
            "summary": r.get("summary"), "practiceMode": r.get("practice_mode"),
...
async def get_practice_record(record_id: int):
    ...
    return {
        "recordId": record.id, "courseId": record.course_id,
        "userName": record.user_name, "totalScore": record.total_score,
        "duration": record.duration, "dialogRounds": record.dialog_rounds if hasattr(record, 'dialog_rounds') else None,
        "reportData": record.report_data if hasattr(record, 'report_data') else None,
...
async def create_practice_record(req: PracticeRecordCreate):
    ...
    return {"message": "Practice record created successfully", "recordId": result.id}
...
async def get_scene_statistics(scene_id: Optional[str] = None):
    stats = await StatisticsService.get_scene_stats(int(scene_id) if scene_id else None)
    return {"scene_stats": [
        {"scene_id": s.course_id, "practice_count": s.practice_count or 0, "avg_score": s.avg_score or 0}
        for s in stats
    ]}
...
async def get_leaderboard(limit: Optional[int] = 10):
    ...
    return {"leaderboard": [
        {"user_id": l.user_name, "practice_count": l.practice_count or 0, "avg_score": round(l.avg_score, 2) if l.avg_score else 0}
